...
# ============ FORECAST ============
    ...
    try:
        ...
        if ai.get("disabled_reason"):
            ...
            st.caption("🔎 AI 会先抓取该商品的**实时国际新闻**(Google News)再分析，"
                       "只根据真实头条、标注来源日期，不靠猜测。"
                       "AI reads real-time news before analysing - grounded, not guessed.")
            if st.button("Generate AI market analysis / 生成 AI 市场分析",
                         type="secondary"):
                with st.spinner(f"📡 抓取实时新闻并询问 {ai['model']}... / "
                                "Fetching live news & asking AI..."):
                    try:
                        ...
                        # Show the real headlines the analysis was grounded in
                        from src.analytics.news import fetch_news
                        news = fetch_news(selected_food)
                        heads = news.get("headlines", [])
                        if heads:
                            with st.expander(
                                    f"📰 分析依据的真实新闻来源 / News sources "
                                    f"used ({len(heads)})", expanded=True):
                                for h in heads:
                                    src = h.get("source") or "source"
                                    st.markdown(
                                        f"- [{h['date']}] [{h['title']}]({h['link']}) "
                                        f"— *{src}*")
                                st.caption("点击标题可打开原文核实 / "
                                           "Click a headline to verify the source.")
                        elif news.get("error"):
                            st.warning(f"实时新闻抓取失败，AI 仅用统计数据分析 / "
                                       f"Live news unavailable ({news['error']}).")
                        st.caption(f"Generated by {ai['provider']} / {ai['model']} - "
                                   "grounded in the real headlines listed above; "
